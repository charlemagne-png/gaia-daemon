...
/** Sticky prompt-later note, display-only room metadata. */
...
export interface RoomState {
  ...
  /** Stable human-speakable workspace-scoped room reference. */
  refCode?: string;
  ...
  /** /love tree mode. */
  love?: true;
  /** /teleport room-local gaiaport link flag. */
  teleport?: boolean;
  /** /love sanitize rebirth pointer. */
  rebirth?: { to: string; at: string };
  /** Count of auto-heal applies that landed in this room. */
  autoHeals?: number;
  ...
  parentRoomId?: string;
  /** User-opened subroom, first-class (not a summon lane). */
  subroom?: boolean;
  ...
  notes?: RoomNote[];
  /** GaiaVoice dispatcher stickiness. */
