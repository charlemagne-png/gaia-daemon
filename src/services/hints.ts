...
function configJsonHints(sources: HintSources): FileHints {
  return {
    ...
    "voice.dispatcherAgentId": { input: "text", optional: true, label: "Voice dispatcher agent", description: "workspace override for GaiaVoice dispatcher (default hermes)" },
    "voice.agentAliases": { input: "json", optional: true, label: "Voice alias map", description: "spoken alias map for tier-0 voice routing, e.g. {\"Alice\":\"artus\"}" },
...
function agentJsonHints(sources: HintSources, parsed?: Record<string, unknown>, agentId?: string, workspaceId?: string): FileHints {
  ...
  return {
    ...
    aliases: { input: "json", optional: true, label: "Voice aliases", description: "spoken names that tier-0 voice routing treats as this agent" },
...
function voiceJsonHints(): FileHints {
  return {
    ...
    dispatcherAgentId: { input: "text", optional: true, label: "Voice dispatcher agent", description: "agent that receives non-addressed GaiaVoice utterances (default hermes); if missing, voice routing falls back unchanged" },
