...
export class RoomCommandsMixin {
  ...
  pluginContext(plugin: CommandPlugin, state: Awaited<ReturnType<RoomHandle["state"]>>, command?: string, stateRoomId = this.roomId): PluginContext {
    return {
      ...
      state: state.pluginState?.[pluginStateKey(plugin)], stateRoomId,
...
${draft.summary}` : ""}`;
  ...
  async runGaiagoCommand(text?: string): Promise<string> {
    const source = text?.trim();
    if (!source) return "usage: /gaiago <text|path>";
    const host = this.options.summonHost;
    if (!host) return "Summons are not available in this workspace.";
  ...
  async runLoveCommand(off?: boolean): Promise<string> {
    await this.room.updateState((state) => {
      ...
      else state.love = true;
    });
    await this.emitSnapshot();
    return off ? "💗 love off." : "💗 love on — this room answers through love.";
  }

  async runTeleportCommand(on?: boolean): Promise<string> {
    await this.room.updateState((state) => {
      if (on === false) delete state.teleport;
      else state.teleport = true;
    });
    await this.emitSnapshot();
    return on === false ? "teleport off — gaiaport link inactive" : "teleport on — gaiaport link active";
  }

  async runLoveSanitizeCommand(): Promise<string> {
    const proposal = await this.sanitizePreview();
    return `💗 Dario reviewed ${proposal.window} message${proposal.window === 1 ? "" : "s"}: ${proposal.suggestions.length} love rewrite${proposal.suggestions.length === 1 ? "" : "s"} ready.`;
  }

  async runLoveSanitizeAutoCommand(): Promise<string> {
    const proposal = await this.sanitizePreview();
    if (!proposal.suggestions.length) return `💗 auto-heal reviewed ${proposal.window} messages; no rewrites needed.`;
    const edits = proposal.suggestions.map((s) => ({ eventId: s.eventId, quote: s.quote, replacement: s.replacement }));
    const result = await this.sanitizeApply(edits);
    await this.room.updateState((state) => { state.autoHeals = (state.autoHeals ?? 0) + 1; });
    return `💗 auto-heal applied ${result.applied} rewrite${result.applied === 1 ? "" : "s"}${result.skipped ? ` (${result.skipped} skipped)` : ""}.`;
  }

  async runLoveSanitizeRebirthCommand(): Promise<string> {
    await this.room.updateState((state) => { state.rebirth = { to: `${this.roomId}-rebirth`, at: new Date().toISOString() }; });
    await this.emitSnapshot();
    return "💗 rebirth queued — this room is marked for a fresh continuation brief.";
  }
